...
def afids_to_fcsv(transformed_coords, output_path, template_path=FCSV_TEMPLATE):
    """
    Writes transformed AFID coordinates to a new FCSV file using a template FCSV.
    ...
    with open(template_path, encoding="utf-8", newline="") as template_file:
        ...
        reader = csv.DictReader(template_file, fieldnames=["id", "x", "y", "z", "ow", "ox", "oy", "oz", "vis", "sel", "lock", "label", "desc", "associatedNodeID"])
        rows = list(reader)
...
def apply_affine_transform(mat_path, coords):
    ...
    transformed_homogeneous = (np.linalg.inv(transformation_matrix) @ homogeneous_coords.T).T
    return transformed_homogeneous[:, :3]

# --- APPLY WARP (HARMONIZED) ---
def apply_warp_deformation(transform_path, coords, flip_ras_lps=True):
    """
    Applies a non-linear warp deformation to coordinates using a SimpleITK displacement field.
...
def make_toggleable_heatmap(error_components, afid_ids):
    ...
    fig.update_layout(height=600, width=500, margin=dict(t=0, b=0, l=0, r=0), template='plotly_white')
...
def make_3d_plot(gt, pred, afid_ids):
    ...
    fig.add_trace(go.Scatter3d(x=gt[:, 0], y=gt[:, 1], z=gt[:, 2],
        mode='markers', name='Stereotactic Space',
        marker=dict(size=6, color='green', opacity=0.7, symbol='diamond'), text=afid_ids))
    fig.add_trace(go.Scatter3d(x=pred[:, 0], y=pred[:, 1], z=pred[:, 2],
        mode='markers', name='Registered Subject',
        marker=dict(size=6, color='red', opacity=0.7, symbol='cross'), text=afid_ids))
    for i in range(gt.shape[0]):
        fig.add_trace(go.Scatter3d(x=[gt[i, 0], pred[i, 0]], y=[gt[i, 1], pred[i, 1]], z=[gt[i, 2], pred[i, 2]],
            mode='lines', line=dict(color='orange', width=7), showlegend=False))
    fig.update_layout(scene=dict(xaxis_title='X', yaxis_title='Y', zaxis_title='Z', bgcolor='rgb(250,250,250)'),
                      height=600, width=800, margin=dict(t=0, b=0, l=0, r=0), template='plotly_white',
                      legend=dict(x=0.7, y=0.9))
    return pio.to_html(fig, full_html=False, include_plotlyjs='cdn')

# --- Coordinate Utilities ---
def world_to_voxel_coords(img, world_coords):
    """Convert world (RAS+) coordinates to voxel coordinates using inverse affine."""
...
def render_mri_planes(data, coords, zoom=50, show_crosshairs=True):
    """
    Render sagittal, coronal, and axial slices centered at given voxel coordinates.
...
def build_afid_viewer_block(label, error_val, subject_image, ref_image):
    ...
    <div class="viewer-card" style="text-align: center">
        ...
        <!-- Header -->
        <div class="viewer-header" style="font-weight: bold; font-size: 18px; margin-bottom: 1px;">
            AFID {label}: {error_val:.2f} mm
        </div>

        <!-- Image container -->
        <div class="fade-image-wrapper" style="position: relative; width: 100%; height: 400px; border-radius: 10px; overflow: hidden;">
            ...
            <img id="fade_{label}" 
                ...
                style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: contain; z-index: 2; opacity: 0; transition: opacity 1s ease-in-out;" />
        </div>

        <!-- Labels below -->
        <div style="margin-top: 0px; font-size: 14px;">
            <span id="label_subject_{label}" style="padding: 3px 10px; background-color: #ddd; border-radius: 6px; font-weight: bold;">Subject Space</span>
            &nbsp;←→&nbsp;
            <span id="label_ref_{label}" style="padding: 3px 10px; background-color: #f0f0f0; border-radius: 6px; color: #999;">Stereotactic Space</span>
        </div>
...
# --- Main Viewer Builder ---
def generate_qualitative_mri_html(subject_nii, ref_nii, ref_fcsv, ed_errors, labels=list(range(1, 33)), zoom=40):
    """Generates HTML for visual comparison of AFID localization on subject vs template MRI."""
    ...
    for i, label in enumerate(labels):
        world_coords, template_voxel = get_afid_world_and_voxel_coords(ref_img, ref_fcsv, label)
    ...
    <div class='card'>
        ...
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <button onclick="scrollAfid(-1)" style="font-size: 20px;">&#8592;</button>
            <div style="flex: 1; overflow: hidden; position: relative;">
                <div class='afid-strip' id="afid-strip" onscroll="updateAfidCounter()">
                    {''.join(viewer_blocks)}
                </div>
                <div id="afid-counter" style="position: absolute; bottom: 10px; right: 16px; background: rgba(255,255,255,0.8); padding: 4px 10px; border-radius: 8px; font-weight: 600;">
                    1 / {len(viewer_blocks)}
                </div>
            </div>
            <button onclick="scrollAfid(1)" style="font-size: 20px;">&#8594;</button>
...
def render_dashboard_html(output_path, heatmap_html, scatter_html, ed_errors, qualitative_html):
    ...
    <body>
        <div id="zoom-wrapper">                        
            ...
            <div class="container">
                ...
                <div class="card">
                    ...
                    <div class="tile-row" style="margin-top: 20px;">
                        <div class="tile">
                            ...
                            <div class="tile-value">{{ '%.2f' % stats['Mean'] }} mm</div>
                        </div>
                        <div class="tile">
                ...
                <div class="card">
                    <div class="section-title">Registration Error Visualization</div>
                    <div class="flex-row">
...
# --- MAIN WRAPPER ---
def generate_afid_qc_dashboard(gt_fcsv_path, pred_fcsv_path, output_html_path, output_fcsv_path, output_csv_path,subject_nii, template_nii, matfile_path, warpfile_path):
    ...
    qualitative_html = generate_qualitative_mri_html(subject_nii, template_nii, pred_fcsv_path, ed,zoom=50)
    return render_dashboard_html(output_html_path, heatmap_html, scatter_html, ed, qualitative_html)
